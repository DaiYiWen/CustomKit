...
+ (BOOL)moveItemAtPathB:(NSString *)path toPath:(NSString *)toPath overwrite:(BOOL)overwrite error:(NSError *__autoreleasing *)error {
    // 先要保证源文件路径存在，不然抛出异常
    if (![self isExistsAtPath:path]) {
//        [NSException raise:@"非法的源文件路径" format:@"源文件路径%@不存在，请检查源文件路径", path];
        return NO;
...
+(NSString *)sizeFormatted:(NSNumber *)size {
    NSInteger sizeb=[size integerValue];
    sizeb=sizeb/2;
    size=[NSNumber numberWithInteger:sizeb];
//    size=size/2
