...
//    _session.sessionPreset = AVCaptureSessionPreset640x480;
    ...
    AVCaptureConnection *videoConn = [_movieOutput connectionWithMediaType:AVMediaTypeVideo];
    [_movieOutput setOutputSettings:@{AVVideoCodecKey:AVVideoCodecTypeH264} forConnection:videoConn];
...
- (void)startSession
{
    if (!self.sessionConfigurateSuccess) return;
...
- (void)addZoomFactorObserver
{
    ...
    self.releaseObserver = true;
}

- (void)removeZoomFactorObserver
{
    if(self.releaseObserver){
        self.releaseObserver = false;
            ...
            if([UIDevice currentDevice].orientation == UIDeviceOrientationFaceUp){
                if([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft){
                    ...
                    if(@available(iOS 13.0,*)){
                        curInterFace = [UIView getCurrentWindow].windowScene.interfaceOrientation;
                    }else{
...
- (NSURL *)uniqueMovieOutputURL
{
    ...
    free(buffer);
    if (dirPath) {
        NSString *filePath = [dirPath stringByAppendingPathComponent:[NSString stringWithFormat:@"movie_%.0f.mov",[[NSDate date] timeIntervalSince1970]]];
