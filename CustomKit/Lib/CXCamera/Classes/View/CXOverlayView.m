...
//    self.cameraStatusView.size = CGSizeMake(self.width, kCXOverlayStatusViewheight + (IS_IPhone_Liu ? 44 : 10));
    self.cameraStatusView.size = CGSizeMake(self.width, kCXOverlayStatusViewheight + 44);
...
{
    if(self.cameraModeView.cancelButton){
        self.cameraModeView.cancelButton.enabled = false;
    }
    if ([self.delegate respondsToSelector:@selector(didSwitchCamera)]) {
        [self.delegate didSwitchCamera];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.cameraModeView.cancelButton.enabled = true;
        });
    }
}
